...
if($_POST) {
    ...
    $emailTo = 'info@atinskiteboarding.com';
    ...
    if($clientName == '') {
        die('Por favor ingrese su nombre');
    }
    if(!isEmail($clientEmail)) {
        die('Por favor ingrese un correo válido');
    }
    if($message == '') {
        die('Por favor ingrese su mensaje');
    }
    if($clientName != '' && isEmail($clientEmail) && $message != '') {
        // Send email
	$transport = Swift_SmtpTransport::newInstance('mx1.hostinger.com.ar',
                                              2525)
             ->setUsername('robot@atinskiteboarding.com')
